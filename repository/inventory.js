  ...
  try {
    ...
    if (Array.isArray(payload.ingredients) && payload.ingredients.length > 0) {
      ...
      for (const [index, ingredient] of payload.ingredients.entries()) {
        try {
          console.log(`Processing ingredient ${index + 1}:`, ingredient.item_name);
          ...
          if (!ingredientItem) {
            console.error('Ingredient item not found:', ingredient.item_name);
            continue;
          }
          console.log('Found ingredient item:', ingredientItem._id);

          const existingInventory = await Inventory.findOne({
            storeId: { $in: payload.storeId },
            item: ingredientItem._id
          });

          if (existingInventory) {
            console.log('Inventory already exists for:', ingredient.item_name);
            continue;
          }
          ...
          const ingredientInventory = {
            item_type: "Product",
            item: ingredientItem._id,
            item_name: ingredientItem.item_name,
            user: payload.user,
            storeId: payload.storeId,
            quantityInStock: ingredient.item_quantity || 0,
            estimatedRestockTime: payload.estimatedRestockTime,
            meal_price: ingredient.set_prices || [],
            item_price: Array.isArray(ingredient.set_prices) && ingredient.set_prices.length > 0
              ? Number(ingredient.set_prices[0].price)
              : 0,
          ...
          console.error(`Failed to process ingredient ${ingredient.item_name}:`, ingError);
        }
      }
    } else {
    ...
    throw {
      error: error,
      message: error.message || "Create inventory operation failed",
      code: 500,
    };
  }
};
