...
const getItems = async (page, filter) => {
  ...
  const itemResponse = await Item.find(query)
...
const getAllItems = async (filter) => {
  ...
  const itemResponse = await Item.find(query)
    .populate("item_categories item_description")
    .populate("store_available");
  return { count: itemResponse.length, items: itemResponse };
};
...
const getOneUserItem = async (filter) => {
  try {
    return await Item.find(filter).populate("item_description item_categories");
  } catch (error) {
    console.log(error);
  }
};
...
const filterItem = async (filter, itemType) => {
  try {
    const query = {
      item_name: { $regex: filter, $options: "i" },
      item_status: {
