        ...
        if (gridfs) {
            ...
            gridfs.files.findOne({filename: req.params.filename}, (err, file)=>{
                ...
                if(file.contentType === 'image/jpeg' || file.contentType === 'image/png' ){
                  ...
                  console.log("Found image; piping response into stream.. ");

                  const readstream = gridfs.createReadStream(file.filename);
                  readstream.pipe(res);
                  // return res.status(200).send(readstream.pipe(res))
                }
                else{
                  console.log("Found something thats not an image; probably binary");
            ...
            //     filename: "tomato.jpg"
            // });
            // const {mongodb } = require("mosngodb");
